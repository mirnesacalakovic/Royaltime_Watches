...
axios.defaults.withCredentials = true; //dopušta coockie
...
const request = {
    ...
    postForm: (url: string, data: FormData) => axios.post(url, data, {headers: {'Content-Type': 'mulitpart/form-data'}}).then(responseBody),
    putForm: (url: string, data: FormData) => axios.put(url, data, {headers: {'Content-Type': 'mulitpart/form-data'}}).then(responseBody)
}
...
const Account = {
    ...
    brandRegister: (values: any) => request.post('account/brandRegister', values),
    currentUser: () => request.get('account/currentUser'),
    fetchAddress: () => request.get('account/savedAddress'),
    verifyEmail: (token: string, email: string) => request.post(`/account/verifyEmail?token=${token}&email=${email}`, {}),
    resendEmailConfirmationLink: (email: string) => request.get(`/account/resendEmailConfirmationLink?email=${email}`),
    requestPasswordReset: (email: string) => request.post(`/account/forgotPassword?email=${email}`, {}),
    resetPassword: (resetPasswordDto: ResetPasswordDto) => request.post(`/account/resetPassword`, resetPasswordDto),
    fetchAllBrands: () => request.get('account/brands'),
}
...
const Wishlist = {
    ...
    add: (productId: number) => request.post(`wishlist/add/${productId}`, {}),
    remove: (productId: number) => request.delete(`wishlist/remove/${productId}`)
}
